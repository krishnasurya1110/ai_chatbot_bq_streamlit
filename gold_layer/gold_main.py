...
def main(delta_table_path, temp_gcs_bucket_main, temp_gcs_bucket_incr, project_id, dataset_id, bq_table_id, inc_bq_table_id):
    ...
    logger = logging_client.logger("delta_to_bigquery") 
    
    logger.log_text("Starting Delta to BigQuery data transfer process.", severity=200)
    ...
    logger.log_text("Spark session initialized successfully.", severity=200)
    ...
    try:
        ...
        logger.log_text(f"Loaded data from Delta table path: {delta_table_path}", severity=200)
    except Exception as e:
        logger.log_text(f"Failed to load Delta table: {str(e)}", severity=500)
        raise e
    
    # Filter Delta table to include only the required columns
    delta_df = delta_df.select(*required_columns)
    logger.log_text("Filtered Delta table to include required columns.", severity=200)
    ...
    try:
        ...
        logger.log_text(f"BigQuery table {bq_table_full_name} data check completed. Data exists: {bq_table_has_data}", severity=200)
    except Exception as e:
        logger.log_text(f"Error accessing BigQuery table: {str(e)}", severity=500)
    ...
    if not bq_table_has_data:
        try:
            delta_df.write.format("bigquery") \
                ...
                .mode("overwrite") \
                .save()
            logger.log_text("BigQuery table has no data. All records from Delta table written to BigQuery.", severity=200)
        except Exception as e:
            logger.log_text(f"Failed to write to BigQuery: {str(e)}", severity=500)
            raise e
    else:
        ...
        logger.log_text(f"Identified {new_records_count} new records to be written to BigQuery.", severity=200)
        ...
        if new_records_count > 0:
            try:
                ...
                new_records_df.select(*required_columns).write.format("bigquery") \
                    ...
                    .mode("append") \
                    .save()
                logger.log_text(f"{new_records_count} new required records written to BigQuery.", severity=200)
            except Exception as e:
                logger.log_text(f"Failed to write new records to BigQuery: {str(e)}", severity=500)
                raise e
            ...
            try:
                ...
                logger.log_text(f"Incremental data written to table {incr_bq_table_full_name} (create or replace).", severity=200)
            except Exception as e:
                logger.log_text(f"Failed to write incremental data to BigQuery: {str(e)}", severity=500)
                raise e
        else:
            logger.log_text("No new records to write to BigQuery. Skipping write operation.", severity=200)
    
    logger.log_text("Delta to BigQuery data transfer process completed.", severity=200)
