...
def handle_validation_failure(df, schema_struct, quarantine_path_good, quarantine_path_bad, is_incremental=False):
    ...
    try:
        logger.log_text("Starting data isolation process", severity=200)
...
def main(source_path, delta_table_path,quarantine_path_good,quarantine_path_bad):
    ...
    logger.log_text(f"Loaded data from source path: {source_path}", severity=200)
    ...
    if not delta_presence:
        try:
            ...
            logger.log_text("Performed full data validation", severity=200)
            ...
            if all(schema_check_results['Success']):
                logger.log_text("Schema validation successful", severity=200)
                if not all(validation_summary['Success']):
                    logger.log_text("Data Validation Failed - Processing for quarantine", severity=400)
                    ...
                    logger.log_text("Successfully wrote data to Delta table", severity=200)
            else:
                logger.log_text("Schema validation failed: Schema changed", severity=400)
                raise ValueError("Schema validation failed - Schema mismatch detected")
        except Exception as e:
            logger.log_text(f"Error in initial data processing: {str(e)}", severity=500)
            raise
        ...
        logger.log_text(f"Read Delta table from path: {delta_table_path}", severity=200)
        ...
        if latest_date_source >= latest_date_delta:
            try:
                logger.log_text(f"Data discrepancy detected: Source ({latest_date_source}) Delta ({latest_date_delta})", severity=200)
                ...
                if all(schema_check_results['Success']):
                    logger.log_text("Schema validation successful",severity=200)
                    if not all(validation_summary['Success']):
                        logger.log_text("Incremental Data Validation Failed - Processing for quarantine", severity=400)
                        success = handle_validation_failure(
                            df_new, 
                            # schema_struct, 
                            updated_schema_struct,
                        ...
                        logger.log_text(f'successfully loaded delta table',severity=200)
                        ...
                        logger.log_text("Successfully merged new data into Delta table", severity=200)
            except Exception as e:
                logger.log_text(f"Error in incremental data processing: {str(e)}", severity=500)
                raise
        else:
            logger.log_text("Data is up-to-date", severity=200)
...
if __name__ == '__main__':
    ...
    main("gs://nyc_subway_data/", "gs://nyc_subway_delta_lake/", "gs://nyc_good_data_bucket/", "gs://nyc_bad_data_bucket/")
