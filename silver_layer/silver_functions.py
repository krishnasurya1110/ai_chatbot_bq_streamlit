...
def get_validations(data_source_name,data_asset_name,suite_name,batch_definition_name,definition_name,df):
    ...
    df_ = pd.read_csv("gs://dataproc-cluster-spark-jars/station_data.csv")
    # print(df_.head())
    ...
    logger.log_text(f'gx: Data source created successfully {data_source_name}',severity=200)

    # Define the name of your data asset

    data_asset = data_source.add_dataframe_asset(name=data_asset_name)
    logger.log_text(f'gx: Data asset created successfully {data_asset_name}',severity=200)

    batch_parameters = {"dataframe": df}
    batch_definition = data_asset.add_batch_definition_whole_dataframe(
    batch_definition_name
    )
    logger.log_text(f'gx: Batch created successfully {batch_definition_name}',severity=200)
    ...
    logger.log_text(f'gx: Expectation suite created successfully {suite_name}',severity=200)
    ...
    for i in expected_columns:
        ...
        logger.log_text(f'gx: Adding {i} to ExpectColumnToExist', severity=200)
    ...
    logger.log_text(f'gx: Adding transit_timestamp to ExpectColumnValuesToMatchRegex', severity=200)

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column='transit_mode',
        value_set=['subway', 'tram', 'staten_island_railway']
    ))
    logger.log_text('gx: Adding transit_mode to ExpectColumnValuesToBeInSet', severity=200)

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column='station_complex_id',
        value_set=list(df_['station_complex_id'])
    ))
    logger.log_text('gx: Adding station_complex_id to ExpectColumnValuesToBeInSet', severity=200)

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column='station_complex',
        value_set=list(df_['station_complex'])
    ))
    logger.log_text('gx: Adding station_complex to ExpectColumnValuesToBeInSet', severity=200)

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column='borough',
        value_set=['Brooklyn', 'Manhattan', 'Bronx', 'Queens', 'Staten Island']
    ))
    logger.log_text('gx: Adding borough to ExpectColumnValuesToBeInSet', severity=200)

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column='payment_method',
        value_set=['metrocard', 'omny']
    ))
    logger.log_text('gx: Adding payment_method to ExpectColumnValuesToBeInSet', severity=200)

    suite.add_expectation(gx.expectations.ExpectColumnValuesToBeInSet(
        column='fare_class_category',
        value_set=fare_class_cat
    ))
    logger.log_text('gx: Adding fare_class_category to ExpectColumnValuesToBeInSet', severity=200)
    ...
    logger.log_text('gx: Adding ridership to ExpectColumnValuesToBeBetween', severity=200)
    ...
    logger.log_text('gx: Adding transfers to ExpectColumnValuesToBeBetween', severity=200)
    ...
    logger.log_text('gx: Adding latitude to ExpectColumnValuesToBeBetween', severity=200)
    ...
    logger.log_text('gx: Adding longitude to ExpectColumnValuesToBeBetween', severity=200)

    # Create Validation Definition
    validation_definition = gx.ValidationDefinition(
        data=batch_definition, suite=suite, name=definition_name
    )
    logger.log_text('gx: Creating validation definition', severity=200)

    # Add Validation Definition to the Data Context
    validation_definition = context.validation_definitions.add(validation_definition)
    logger.log_text('gx: Added validation definition to context', severity=200)

    # Validate the Batch
    validation_results = batch.validate(suite)
    logger.log_text('gx: Validation executed', severity=200)
    ...
    logger.log_text('gx: Processed validation results', severity=200)

    # Convert Results to DataFrame
    res_df = pd.DataFrame(results)
    logger.log_text('gx: Converted validation results to DataFrame', severity=200)
...
def data_isolation(df):
    ...
    for condition, result in condition_results.items():
        ...
        logger.log_text(f"Condition '{condition}': Success={success_count}, Failure={failure_count}", severity=200)
    ...
    logger.log_text(f"Bad records proportion: {bad_records_df.count()/df.count()}", severity=200)

    # Filter good records (optional, if needed)
    good_records_df = df.filter(~bad_records_condition)
    logger.log_text(f"Good records proportion: {good_records_df.count()/df.count()}", severity=200)
...
def check_delta_existance(spark, delta_table_path):
    if DeltaTable.isDeltaTable(spark, delta_table_path):
        logger.log_text(f"Delta table exists at path: {delta_table_path}", severity=200)
        return True
    else:
        logger.log_text(f"Delta table does not exist at path: {delta_table_path}", severity=500)
        return False
